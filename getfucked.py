...
BASE_URL = 'https://api.x.com/v1/users/'  # Replace with the actual API base URL
...
def download_profile_data(username, output_format='json'):
    ...
    try:
        ...
        # Log the metadata
        logger.info(f"X data for {username} (1 of 1)")
...
def download_multiple_users(usernames, output_format='json'):
    for username in usernames:
        download_profile_data(username, output_format)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Download X.com profile data for specified usernames.')
    parser.add_argument('usernames', type=str, nargs='+', help='The usernames of the profiles to download data for')
    parser.add_argument('--output', type=str, choices=['json', 'txt'], default='json', help='Output format (json or txt)')

    args = parser.parse_args()
    
    download_multiple_users(args.usernames, args.output)
